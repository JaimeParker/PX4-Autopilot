...
- **PWM visual confirmation:** The plot below overlays PX4-logged PWM (blue, staircase at ~10 Hz publish rate) against extraction-computed PWM (orange, 100 Hz). Both signals track the same envelope; per-sample mismatches are caused by the stale nearest-timestamp hold between `actuator_outputs` publishes.

![L3 PWM Comparison](l3_pwm_comparison.png)

*Generated by `scripts/plot_pwm_comparison.py`. Subplot 5 shows the per-motor PWM error — spikes cluster at manoeuvre transients where timestamp misalignment is largest.*
...
```
l3_replay_gtest: 4 tests — ALL PASS (75 ms)
```
